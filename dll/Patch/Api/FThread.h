...
{
public:
	Sonicteam::SoX::LinkNode<FThread> m_lnThread;
...
public:
	FThread(const char* Name, uint32_t Proccessor, bool IsSyncronize);
	~FThread();

	MARATHON_DESTRUCTION_H(FThread);
	virtual void Syncronize();
	virtual void Run();
	virtual void OnStep(double);
	
	// kernel
	void __ThreadKernelStep();
};