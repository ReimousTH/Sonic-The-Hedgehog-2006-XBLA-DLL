...
void cpp_hl_tests(RegEx& e, bool recurse = true)
{
   ...
   else
   {
      if(e.Search(search_text.c_str(), flags[3]))
      {
         ...
         while(matches[j] != -2)
         {
            if( (matches[j] != (int)e.PostPosition(i))
               || ((matches[j] != -1) && ((matches[j+1] - matches[j] != (int)e.Length(i)))) )
            {
               begin_error();
               cout << "RegEx::Search error in subexpression " << i << ": found [" << e.PostPosition(i) << "," << (e.PostPosition(i) + e.Length(i)) << "] expected [" << matches[j] << "," << matches[j+1] << "]" << endl;
            }
            ++i;
            j += 2;
         }
      ...
      if((matches[0] == 0) && (matches[1] == search_text.size()))
      {
         if(e.Match(search_text.c_str(), flags[3]))
         {
            ...
            while(matches[j] != -2)
            {
               ...
               {
                  begin_error();
                  cout << "RegEx::Match error in subexpression " << i << ": found [" << e.PostPosition(i) << "," << (e.PostPosition(i) + e.Length(i)) << "] expected [" << matches[j] << "," << matches[j+1] << "]" << endl;
               }
               ++i;
               j += 2;
            }
