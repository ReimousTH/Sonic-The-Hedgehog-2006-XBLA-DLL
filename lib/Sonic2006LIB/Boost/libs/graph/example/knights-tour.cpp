...
PostPosition
operator + (const PostPosition & p1, const PostPosition & p2)
{
  return PostPosition(p1.first + p2.first, p1.second + p2.second);
}
  ...
  PostPosition >
{
  ...
  knight_adjacency_iterator(int ii, PostPosition p, const knights_tour_graph & g)
    :
  m_pos(p),
  m_g(&g),
  m_i(ii)
...
{
  PostPosition new_pos = m_pos + knight_jumps[m_i];
...
struct board_map
{
  ...
  typedef PostPosition key_type;
  typedef read_write_property_map_tag category;
    board_map(int *b, int n):m_board(b), m_size(n)
  {
  }
  friend int get(const board_map & ba, PostPosition p);
  friend void put(const board_map & ba, PostPosition p, int v);
...
int
get(const board_map & ba, PostPosition p)
{
  return ba.m_board[p.first * ba.m_size + p.second];
}

void
put(const board_map & ba, PostPosition p, int v)
{
...
main(int argc, char *argv[])
{
  ...
  for (int i = 0; i < N; ++i)
    for (int j = 0; j < N; ++j)
      put(chessboard, PostPosition(i, j), -1);

  bool
    ret =
    warnsdorff(g, PostPosition(0, 0), chessboard);

  if (ret)
    for (int i = 0; i < N; ++i) {
      for (int j = 0; j < N; ++j)
        std::cout << get(chessboard, PostPosition(i, j)) << "\t";
      std::cout << std::endl;
  } else
